...
const login = asyncHundler(async (req, res) => {
    ...
    if (!user) {
        res.status(402).json({ status: "fail", message: "user not found" });
    }

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
        res.status(403).json({ status: "fail", message: "password not match" });
    }
    ...
    const token = await jwt.sign({ user: user._id, }, process.env.JWT, { expiresIn: "1h" })
